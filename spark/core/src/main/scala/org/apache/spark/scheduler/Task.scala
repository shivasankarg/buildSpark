...
private[spark] abstract class Task[T](
    ...
    @transient var localProperties: Properties = new Properties) extends Serializable with Logging{
  ...
  final def run(
    ...
    context = new TaskContextImpl(
      ...
      metrics)
    if (SHIVAlog == 1) {
      logInfo("SHIVA LOG: in Task.Run()")
      logInfo(s"SHIVA LOG: stageId = $stageId, partitionId = $partitionId")
      logInfo(s"SHIVA LOG: taskAttemptId = $taskAttemptId, attemptNumber = $attemptNumber")
      println(s"SHIVA LOG: taskAttemptId = $taskAttemptId, attemptNumber = $attemptNumber")
    }
    ...
    try {
      if (SHIVAlog == 1) {
        logInfo("SHIVA LOG: Calling runTask(context)")
      }
      runTask(context)
    } catch {
      case e: Throwable =>
