...
private[spark] class PythonRDD(
  ...
  var timesList : List[Double] = List()
  ...
  override def compute(split: Partition, context: TaskContext): Iterator[Array[Byte]] = {
    if (SHIVAlog == 1) {
      logInfo("SHIVA LOG: in PythonRDD compute")
    }
...
private[spark] class PythonRunner(
  ...
  var wstt : Long = System.currentTimeMillis
  var wedt : Long = System.currentTimeMillis
  var rstt : Long = System.currentTimeMillis
  var redt : Long = System.currentTimeMillis
  ...
  def compute(
    ...
    if (SHIVAlog == 1) {
      logInfo("SHIVA LOG: In PythonRunner compute")
    }
    ...
    writerThread.start()
    if (SHIVAlog == 1) {
      logInfo("SHIVA LOG: Writer Thread Started")
    }
    ...
    if (SHIVAlog == 1) {
      // logInfo("SHIVA LOG: tempCount is " + tempCount)
      redt = System.currentTimeMillis
      logInfo("SHIVA LOG: Read End Time = " + redt)
  ...
  class WriterThread(
    ...
    if (SHIVAlog == 1) {
      logInfo("SHIVA LOG: Writer Thread created")
    }
    ...
    override def run(): Unit = Utils.logUncaughtExceptions {
      try {
        if (SHIVAlog == 1) {
          logInfo("SHIVA LOG: Writer Thread running")
        }
        ...
        if (isUDF) {
          ...
          if (SHIVAlog == 1) {
            logInfo("SHIVA LOG: funcs length=" + funcs.length)
          }
          funcs.zip(argOffsets).foreach { case (chained, offsets) =>
            dataOut.writeInt(offsets.length)
            if (SHIVAlog == 1) {
              logInfo("SHIVA LOG: offset length=" + offsets.length)
              logInfo("SHIVA LOG: offset =" + offsets)
            }
            ...
            if (SHIVAlog == 1) {
              logInfo("SHIVA LOG: chained funcs length =" + chained.funcs.length)
            }
          ...
          if (SHIVAlog == 1) {
            logInfo("SHIVA LOG: command length =" + command.length)
            logInfo("SHIVA LOG: command = " + command)
          }
        ...
        // Data values
        if (SHIVAlog == 1) {
          logInfo("SHIVA LOG: Writer Thread - starting write iterator to stream")
          wstt = System.currentTimeMillis
          logInfo("SHIVA LOG: WT Iterator write Start Time = " + wstt)
        }
        PythonRDD.writeIteratorToStream(inputIterator, dataOut)
        if (SHIVAlog == 1) {
          logInfo("SHIVA LOG: Writer Thread - written iterator to stream")
          wedt = System.currentTimeMillis
          logInfo("SHIVA LOG: WT Iterator write End Time = " + wedt)
          logInfo("SHIVA LOG: Time to write iterator is " + (wedt - wstt)/1000)
        }
...
private[spark] object PythonRDD extends Logging {
  ...
  def runJob(
    ...
    logInfo("SHIVA LOG: PythonRDD runJob")
  ...
  JavaRDD[Array[Byte]] = {
    if (SHIVAlog == 1) {
      logInfo("SHIVA LOG: READING RDD FROM FILE")
    }
    logInfo("SHIVA LOG: READING RDD FROM FILE")
  ...
  def writeIteratorToStream[T](iter: Iterator[T], dataOut: DataOutputStream) {
    ...
    def write(obj: Any): Unit = obj match {
      ...
      case arr: Array[Byte] =>
        ...
        //     var strn = new String(arr,"UTF-8")
        //     logInfo("SHIVA LOG: Written Iterator DATA array[Byte] is " + strn)
        //     var t : Long = System.currentTimeMillis
        //     logInfo("SHIVA LOG: WT Iterator write 1 = " + t)
        //   }
        // }
        // if(tempCount1 == 100){
        //   if (SHIVAlog == 1) {
      ...
      case stream: PortableDataStream =>
        ...
        if (SHIVAlog == 1) {
          logInfo("SHIVA LOG: writerThread Stream")
        }
      case (key, value) =>
        write(key)
        write(value)
