...
private[spark] class BlockManager(
  ...
  private def getLocationBlockIds(blockIds: Array[BlockId]): Array[Seq[BlockManagerId]] = {
    ...
    if (SHIVAlog == 1) {
      logInfo("SHIVA LOG: getLocationBlockIDs")
    }
    locations
  }
  ...
  def getLocalValues(blockId: BlockId): Option[BlockResult] = {
    ...
    if (SHIVAlog == 1) {
      logInfo("SHIVA LOG: getLocalValues")
    }
    blockInfoManager.lockForReading(blockId) match {
      case None =>
        logDebug(s"Block $blockId was not found")
        logInfo(s"Block $blockId was not found")
        None
      case Some(info) =>
  ...
  private def doGetLocalBytes(blockId: BlockId, info: BlockInfo): ChunkedByteBuffer = {
    ...
    logDebug(s"Level for block $blockId is $level")
    if (SHIVAlog == 1) {
      logInfo("SHIVA LOG: doGetLocalBytes")
    }
  ...
  private def doPutIterator[T](
    ...
    doPut(blockId, level, classTag, tellMaster = tellMaster, keepReadLock = keepReadLock) { info =>
      ...
      if (SHIVAlog == 1) {
        logInfo("SHIVA LOG: doPutIterator")
      }
