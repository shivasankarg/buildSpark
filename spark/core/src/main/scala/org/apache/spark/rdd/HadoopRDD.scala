...
class HadoopRDD[K, V](
  ...
  override def compute(theSplit: Partition, context: TaskContext): InterruptibleIterator[(K, V)] = {
    val iter = new NextIterator[(K, V)] {
      ...
      if (SHIVAlog == 1) {
        logInfo("SHIVA LOG: Reading one Partition")
      }
      val jobConf = getJobConf()
      ...
      split.inputSplit.value match {
        case fs: 
          FileSplit => InputFileNameHolder.setInputFileName(fs.getPath.toString)
          if (SHIVAlog == 1) {
            logInfo("SHIVA LOG: Input File name = " + fs.getPath)
          }
    ...
    if (SHIVAlog == 1) {
      logInfo("SHIVA LOG: Exiting HadoopRDD compute")
    }
...
private[spark] object HadoopRDD extends Logging {
  ...
  private[spark] class HadoopMapPartitionsWithSplitRDD[U: ClassTag, T: ClassTag](
    ...
    override def compute(split: Partition, context: TaskContext): Iterator[U] = {
      ...
      if (SHIVAlog == 1) {
        logInfo("SHIVA LOG: HadoopMapPartitionsWithSplitRDD")
      }
