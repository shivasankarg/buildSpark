  ...
  class TaskRunner(
    ...
    override def run(): Unit = {
      ...
      try {
        ...
        // fileHandler.write(taskId.toString+" : ")
        // fileHandler.close()
        // logInfo("SHIVA LOG: in TaskRunner writing to file : " + taskId.toString+" : ")

        logDebug("Task " + taskId + "'s epoch is " + task.epoch)
        env.mapOutputTracker.updateEpoch(task.epoch)

        if (SHIVAlog == 1) {
          logInfo("SHIVA LOG: in TaskRunner")
        }
        ...
        if (SHIVAlog == 1) {
          logInfo("SHIVA LOG: After Task.run(....)")
          logInfo("SHIVA LOG: After Task.run , task exec time is : " + ((taskFinish - taskStart) - task.executorDeserializeTime))
        }
