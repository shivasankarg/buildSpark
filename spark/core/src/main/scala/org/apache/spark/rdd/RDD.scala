...
abstract class RDD[T: ClassTag](
  ...
  var listTimes : List[Double] = List()
  ...
  final def dependencies: Seq[Dependency[_]] = {
    if (SHIVAlog == 1) {
      logInfo("SHIVA LOG: In dependencies")
    }
  ...
  final def iterator(split: Partition, context: TaskContext): Iterator[T] = {
    if (storageLevel != StorageLevel.NONE) {
      if (SHIVAlog == 1) {
        logInfo("SHIVA LOG: Calling getOrCompute")
      }
      getOrCompute(split, context)
    } else {
      if (SHIVAlog == 1) {
        logInfo("SHIVA LOG: Calling computeOrReadCheckpoint")
      }
  ...
  private[spark] def computeOrReadCheckpoint(split: Partition, context: TaskContext): Iterator[T] =
  {
    if (isCheckpointedAndMaterialized) {
      ...
      if (SHIVAlog == 1) {
        logInfo("SHIVA LOG: Calling compute inside computeOrReadCheckpoint")
      }
  ...
  def flatMap[U: ClassTag](f: T => TraversableOnce[U]): RDD[U] = withScope {
    val cleanF = sc.clean(f)
    if (SHIVAlog == 1) {
      logInfo("SHIVA LOG: In flatMap")
    }
    // Iterator[T] ttt = new MapPartitionsRDD[U, T](this, (context, pid, iter) => iter)
    // ttt.foreach{println}
    // ttt.flatMap(cleanF)
  ...
  def filter(f: T => Boolean): RDD[T] = withScope {
    ...
    //   preservesPartitioning = true).foreach{println}
    new MapPartitionsRDD[T, T](
      this,
      (context, pid, iter) => iter.filter(cleanF),
  ...
  def mapPartitions[U: ClassTag](
    ...
    if (SHIVAlog == 1) {
      logInfo("SHIVA LOG: In mapPartitions")
    }
  ...
  private[spark] def mapPartitionsInternal[U: ClassTag](
      ...
      preservesPartitioning: Boolean = false): RDD[U] = withScope {
    if (SHIVAlog == 1) {
      logInfo("SHIVA LOG: In mapPartitionsInternal")
    }
    new MapPartitionsRDD(
      this,
  ...
  def mapPartitionsWithIndex[U: ClassTag](
    ...
    val cleanedF = sc.clean(f)
    if (SHIVAlog == 1) {
      logInfo("SHIVA LOG: In mapPartitionsWithIndex")
    }
    new MapPartitionsRDD(
      this,
  ...
  def saveAsTextFile(path: String): Unit = withScope {
    ...
    // println("before saveAsHadoopFile timeList is " + PythonRDD.timesList)
    val temp = RDD.rddToPairRDDFunctions(r)(nullWritableClassTag, textClassTag, null)
      .saveAsHadoopFile[TextOutputFormat[NullWritable, Text]](path)
    // println("after saveAsHadoopFile timeList is " + PythonRDD.timesList)
    val totalTime : Double = PythonRDD.timesList.sum
  ...
  protected[spark] def firstParent[U: ClassTag]: RDD[U] = {
    if (SHIVAlog == 1) {
      logInfo("SHIVA LOG: In firstParent")
    }
